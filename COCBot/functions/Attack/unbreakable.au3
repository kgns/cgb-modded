...
Func Unbreakable()
	...
	; Set absolute minimum loot required to still farm for more loot, and min loot that will atttact enemy attackers
	;
	Local $x, $y, $iTime

	Switch $iUnbreakableMode
		Case 2
			If $GoldVillage > $iUnBrkMaxGold And $ElixirVillage > $iUnBrkMaxElixir Then
	...
	If (($GoldVillage - $iUnBrkMinGold) < 0) Or (($ElixirVillage - $iUnBrkMinElixir) < 0) Then
		SetLog("Oops, Out of loot - back to farming", $COLOR_RED)
